...
char ip_server[][32] = {
    "10.1.0.12","10.1.0.28","10.1.0.44","10.1.0.60",
    "10.1.0.76","10.1.0.92","10.1.0.108","10.1.0.124",
};
...
/********* Packet related *********/
uint32_t req_id = 0;
int max_req_id = 0xFFFFFFFF;
uint32_t req_id_core[8] = {0};

/********* Results *********/
uint64_t pkt_sent = 0;
uint64_t pkt_recv = 0;
LatencyResults latency_results = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0};
...
static void generate_packet(uint32_t lcore_id, struct rte_mbuf *mbuf,
  ...
  uint8_t server_index = (uint8_t)rte_rand() % 8;
  ...
  // req->req_id = req_id;
  req->req_id = req_id_core[lcore_id];
