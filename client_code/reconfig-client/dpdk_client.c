...
uint64_t high_sleep_time = 10;
uint64_t low_sleep_time = 30;
uint32_t high_qps = 720000;
uint32_t low_qps = 500000;
/*
...
static void generate_packet(uint32_t lcore_id, struct rte_mbuf *mbuf,
  ...
  // req->req_id = req_id;
  req->req_id = htonl((req->client_id << 25) + req_id_core[lcore_id]);
...
static void process_packet(uint32_t lcore_id, struct rte_mbuf *mbuf) {
  ...
  uint64_t reply_run_ns = rte_le_to_cpu_64(res->gen_ns);
  // printf("no conversion %"PRIu64",%"PRIu64"\n", res->gen_ns, reply_run_ns);
  assert(cur_ns > res->gen_ns);
  uint64_t sjrn = cur_ns - res->gen_ns;  // diff in time
  pkt_recv++;
  // sample
  uint64_t sample_count = qps/100000;
  // uint64_t sample_count = qps/10000;
...
static void exp_tx_loop(uint32_t lcore_id, double work_mu) {
  ...
  uint64_t second_tsc = rte_get_tsc_hz();
  uint64_t high_start_tsc = cur_tsc + second_tsc*high_sleep_time;
  uint64_t low_start_tsc = cur_tsc + second_tsc*low_sleep_time;
  uint8_t set_high_flag = 0;
  uint8_t set_low_flag = 0;

  while (1) {
    if ((rte_rdtsc() > high_start_tsc) && (set_high_flag==0)) {
      qps = high_qps;
      lambda = qps * 1e-9;
      mu = 1.0 / lambda;
      init_exp_dist(dist, mu);
      set_high_flag = 1;
      printf("set sending rate to high_qps...\n");
    }
    if ((rte_rdtsc() > low_start_tsc) && (set_low_flag==0)) {
      qps = low_qps;
      lambda = qps * 1e-9;
      mu = 1.0 / lambda;
      init_exp_dist(dist, mu);
      set_low_flag = 1;
      printf("set sending rate to low_qps...\n");
    }
