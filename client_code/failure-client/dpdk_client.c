...
static void dump_stats_to_file() {
  // write the real-time tput into file
  FILE *tput_f = fopen("./results/tput", "wb");
  fwrite(throughput_per_second, sizeof(uint64_t), second, tput_f);
  fclose(tput_f);
...
static void rx_loop(uint32_t lcore_id) {
  ...
  uint64_t update_tsc_step = rte_get_tsc_hz()/10;
  printf("tsz_hz:%"PRIu64",tsz_hz_10:%"PRIu64"\n",update_tsc,update_tsc_step);
  uint64_t next_update_tsc = cur_tsc + update_tsc_step;
  ...
  while (1) {
    ...
    if (unlikely(cur_tsc > next_update_tsc)) {
        ...
        next_update_tsc += update_tsc_step;
    }
