...
struct throughput_statistics tput_stat[MAX_LCORES];
struct throughput_statistics tput_stat_avg[MAX_LCORES];
...
static void print_per_core_throughput(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        throughput_per_second[second] += (tput_stat[i].rx - tput_stat[i].last_rx);
